    ...
    for ele in soccer_games:
        ...
        # 필요한 정보 추출 후 DB에 저장
            ...
            if (hometeam_info[0].get_text() in db_access.get_home_data()) and (current_time in db_access.get_time_data()):
                ...
                # 배당 차이 계산 후 조건에 맞는다면
                if ((db_access.get_home_odds(hometeam_info[0].get_text()) - odds_change_to_float(hometeam_info[1].get_text())) >= 0.26) or ((db_access.get_away_odds(hometeam_info[0].get_text()) - odds_change_to_float(awayteam_info[0].get_text())) >= 0.26):
                    # 해당 아이템 DB에서 삭제
                    db_access.delete_game(hometeam_info[0].get_text(), (date + start_time).rstrip())
                    # 알람 발송
                    client = boto3.client("ses")
                    subject = hometeam_info[0].get_text()
                    body = (date + start_time).rstrip()
                    message = {"Subject": {"Data": subject}, "Body": {"Html": {"Data": body}}}
                    response = client.send_email(Source = [이메일 주소 입력], Destination = {"ToAddresses": [이메일 주소 입력]}, Message = message)
                else:
                    db_access.delete_game(hometeam_info[0].get_text(), (date + start_time).rstrip())
                    ele.next_sibling
            else:
